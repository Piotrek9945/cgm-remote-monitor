...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    for (var t=0; t<treatments.length; t++) {
        var tr = treatments[t];
        if (datastorage.foodeventtypechecked === true && datastorage.foodeventtype === 'food' && tr.eventType !== 'food') { continue; }
        if (tr.eventType === 'food' && datastorage.foodfiltername == null) {
            if (tr == null ) {
                continue;
            } else {
                if (tr.boluscalc == null || tr.boluscalc.foods == null) {
                  continue;
                }
                tr.boluscalc.foods.forEach(function(el) {
                  tr.notes = 'nazwa: ' + el.name + ', porcja: ' + el.portions + ' ' + el.units + ', liczba porcji: ' + el.portionCount + ', carbs: ' + el.carbs + ', eCarbs: ' + el.ecarbs;
        ...
        if (datastorage.foodfiltername != null) {
            if (tr == null || tr.eventType !== 'food') {
                continue;
            } else {
                if (tr == null || tr.boluscalc == null || tr.boluscalc.foods == null) {
                  continue;
                }
                var found = tr.boluscalc.foods.find(function(el) {
                    return el._id === datastorage.foodfiltername
                })
                if (found === undefined) {
                  continue;
                } else {
                    tr.notes = 'nazwa: ' + found.name + ', porcja: ' + found.portions + ' ' + found.units + ', liczba porcji: ' + found.portionCount + ', carbs: ' + found.carbs + ', eCarbs: ' + found.ecarbs;
                }
            }
        }
      var carbs = tr.carbs ? tr.carbs : ''
