...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    for (var t=0; t<treatments.length; t++) {
      var tr = treatments[t];
      var found = tr.findIndex(function(el) {
          return el.boluscalc.foods.findIndex(function (it) {
              return it._id === datastorage.foodfiltername
          }) !== -1
      })
      if (datastorage.foodfiltername == null || found == -1) {
          continue;
      }
